...
//https://github.com/f4exb/picc1101/blob/master/radio.c

void cc1101_set_gd_pin(void){
	rcc_periph_clock_enable(RCC_GPIOA);
	gpio_set_mode(GD_PORT, GPIO_MODE_INPUT,  GPIO_CNF_INPUT_FLOAT, GDO0_PIN | GDO2_PIN);
}
...
void cc1101_reset(void){
	...
	gpio_set_mode(SPI2_A_PORT, GPIO_MODE_OUTPUT_50_MHZ,  GPIO_CNF_OUTPUT_PUSHPULL, SPI2_A_CS);
	spi_set_cs(SPI2, 1);

	/*Reset SPI pin setup, and set pins as GPIO*/
        rcc_periph_clock_disable(RCC_SPI2); //Disable SPI if it was set before
	gpio_set_mode(SPI2_A_PORT, GPIO_MODE_OUTPUT_50_MHZ,  GPIO_CNF_OUTPUT_PUSHPULL, SPI2_A_MOSI | SPI2_A_SCK);
	gpio_set_mode(SPI2_A_PORT, GPIO_MODE_INPUT,  GPIO_CNF_INPUT_FLOAT, SPI2_A_MISO);
...
uint32_t cc1101_calculate_frequency_word(uint32_t xtal_freq, uint32_t trx_freq){
	uint64_t fw;
	fw = ((uint64_t)trx_freq * (uint64_t)(1<<16)) / ((uint64_t)xtal_freq);
	return (uint32_t)fw;
}

uint32_t cc1101_calculate_if_word(uint32_t xtal_freq, uint32_t if_freq){
	return (if_freq * (1<<10)) / xtal_freq;
}
...
void cc1101_calculate_data_rate_word(cc1101_param* param){
	double drate, deviat, f_xtal;
	drate *= param->rate_mult;


}

void cc1101_setup_configuration(cc1101_param* param){
    	cc1101_write_reg(param->device, CC1101_IOCFG0, CC1101_GDPIN_SYNC_HANDLING_SIGNAL); //GD00 now asserts when sync word has been sent/received, and de-asserts at the end of the packet 
    	cc1101_write_reg(param->device, CC1101_IOCFG2, CC1101_GDPIN_SERIAL_CLOCK_SIGNAL); //GD02 now serial clock.synchronous to the data in synchronous serial mode
